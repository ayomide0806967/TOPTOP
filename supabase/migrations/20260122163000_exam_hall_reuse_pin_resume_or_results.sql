...
------------------------------------------------------------------------------
-- Examination Hall: PIN reuse should resume in-progress attempts,
-- or redirect to results when already submitted.
------------------------------------------------------------------------------
...
begin
  ...
  if v_attempt.id is not null then
    ...
    -- If already completed, return a lightweight payload so the client can redirect to results.
...
begin
  ...
  -- If already completed, return without questions so the client can redirect to results.
  if v_attempt.completed_at is not null then
    return jsonb_build_object(
      'status', 'completed',
      'quiz', jsonb_build_object(
