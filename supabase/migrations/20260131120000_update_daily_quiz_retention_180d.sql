-- Update daily quiz retention policy to 180 days (global)
-- - Updates cleanup function default keep-days from 90 -> 180
-- - Replaces any existing pg_cron cleanup jobs that still run 90-day cleanup
-- - Ensures a stable job name for 180-day cleanup

set check_function_bodies = off;

-------------------------------------------------------------------------------
-- Update function default to 180 days
-------------------------------------------------------------------------------

create or replace function public.cleanup_old_daily_quizzes(
  p_keep_days integer default 180,
  p_dry_run boolean default false
)
returns table(
  deleted_quizzes integer,
  deleted_questions bigint,
  deleted_orphans bigint
)
language plpgsql
security definer
set search_path = public
as $$
declare
  v_cutoff date := current_date - make_interval(days => p_keep_days);
  v_deleted_quizzes integer := 0;
  v_deleted_questions bigint := 0;
  v_deleted_orphans bigint := 0;
begin
  if p_dry_run then
    return query
      select 0::integer,
             (
               select count(*)
               from public.daily_quiz_questions dqq
               join public.daily_quizzes dq on dq.id = dqq.daily_quiz_id
               where dq.assigned_date < v_cutoff
             )::bigint,
             (
               select count(*)
               from public.daily_quiz_questions d
               where not exists (
                 select 1 from public.daily_quizzes q where q.id = d.daily_quiz_id
               )
             )::bigint;
    return;
  end if;

  -- Delete child rows first for better performance (parent has cascade too)
  delete from public.daily_quiz_questions dqq
  using public.daily_quizzes dq
  where dqq.daily_quiz_id = dq.id
    and dq.assigned_date < v_cutoff;
  get diagnostics v_deleted_questions = row_count;

  -- Delete old quizzes
  delete from public.daily_quizzes dq
  where dq.assigned_date < v_cutoff;
  get diagnostics v_deleted_quizzes = row_count;

  -- Safety: remove any orphaned question rows
  delete from public.daily_quiz_questions d
  where not exists (
    select 1 from public.daily_quizzes q where q.id = d.daily_quiz_id
  );
  get diagnostics v_deleted_orphans = row_count;

  return query select v_deleted_quizzes, v_deleted_questions, v_deleted_orphans;
end;
$$;

grant execute on function public.cleanup_old_daily_quizzes(integer, boolean) to authenticated;

-------------------------------------------------------------------------------
-- Update cron job to run 180-day retention (03:30 UTC nightly)
-------------------------------------------------------------------------------

do $$
declare
  job record;
begin
  if exists (select 1 from pg_available_extensions where name = 'pg_cron') then
    create extension if not exists pg_cron;

    -- Remove any legacy 90-day cleanup jobs to avoid silently deleting too much.
    for job in
      select j.jobid
      from cron.job j
      where j.command ilike '%cleanup_old_daily_quizzes(90%'
         or j.jobname = 'daily_quiz_cleanup_90d'
    loop
      perform cron.unschedule(job.jobid);
    end loop;

    -- Also remove any existing 180-day job with the same name (idempotent migrations).
    for job in
      select j.jobid
      from cron.job j
      where j.jobname = 'daily_quiz_cleanup_180d'
    loop
      perform cron.unschedule(job.jobid);
    end loop;

    -- Schedule the 180-day cleanup job.
    perform cron.schedule(
      'daily_quiz_cleanup_180d',
      '30 3 * * *',
      $job$select public.cleanup_old_daily_quizzes(180, false);$job$
    );
  end if;
exception
  when undefined_table then
    -- cron.job may not exist even if pg_cron is present/enabled in some environments
    null;
  when others then
    -- Keep migration idempotent and non-blocking in environments without pg_cron
    null;
end
$$;
