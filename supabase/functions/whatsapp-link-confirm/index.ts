...
async function checkTwilioVerifyWhatsApp(toE164: string, code: string) {
...
serve(async (req) => {
  ...
  try {
    ...
    await checkTwilioVerifyWhatsApp(normalizedPhone, code);

    const admin = createClient(supabaseUrl, serviceKey);

    // Ensure phone is not linked to a different profile.
    ...
    // Update auth user phone so Supabase phone OTP login works.
    ...
    console.error('[whatsapp-link-confirm] Unexpected error:', error);
    return json(500, {
