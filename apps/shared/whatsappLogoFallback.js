const WHATSAPP_LOGO_FALLBACK_SVG = `
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32" aria-hidden="true">
  <defs>
    <linearGradient id="whatsappGradientFallback" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="#25D366" />
      <stop offset="1" stop-color="#128C7E" />
    </linearGradient>
  </defs>
  <rect x="1" y="1" width="30" height="30" rx="7" fill="url(#whatsappGradientFallback)" />
  <path
    fill="#ffffff"
...
const WHATSAPP_LOGO_FALLBACK_URI = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(
  WHATSAPP_LOGO_FALLBACK_SVG.trim()
)}`;

export function installWhatsAppLogoFallbacks(root = document) {
  const logos = root.querySelectorAll(
    'img[src*="whatsapp-logo"], img[data-logo="whatsapp"]'
  );

  logos.forEach((img) => {
    if (!(img instanceof HTMLImageElement)) return;
    if (img.dataset.whatsappFallbackBound === 'true') return;

    img.dataset.whatsappFallbackBound = 'true';
    img.addEventListener('error', () => {
      if (img.dataset.whatsappFallbackApplied === 'true') return;
      img.dataset.whatsappFallbackApplied = 'true';
      img.src = WHATSAPP_LOGO_FALLBACK_URI;
    });
  });
}
